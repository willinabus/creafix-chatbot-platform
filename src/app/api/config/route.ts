...
import type { ChatbotConfig as AppChatbotConfig } from "@/features/chatbot/types";

function valueOrDefault<T>(value: T | null | undefined, fallback: T): T {
  return value ?? fallback;
}

function flattenConfig(apiConfig: Partial<AppChatbotConfig>) {
  const d = defaultChatbotConfig;
  return {
    id: apiConfig.id || 'clarissa-v1',
    name: valueOrDefault(apiConfig.branding?.name, d.branding.name),
    companyName: valueOrDefault(apiConfig.branding?.companyName, d.branding.companyName),
    tagline: valueOrDefault(apiConfig.branding?.tagline, d.branding.tagline),
    logoUrl: valueOrDefault(apiConfig.branding?.logoUrl, d.branding.logoUrl),
    avatarUrl: valueOrDefault(apiConfig.branding?.avatarUrl, d.branding.avatarUrl),
    welcomeMessage: valueOrDefault(apiConfig.branding?.welcomeMessage, d.branding.welcomeMessage),
    inputPlaceholder: valueOrDefault(apiConfig.branding?.inputPlaceholder, d.branding.inputPlaceholder),
    primaryColor: valueOrDefault(apiConfig.style?.primaryColor, d.style.primaryColor),
    secondaryColor: valueOrDefault(apiConfig.style?.secondaryColor, d.style.secondaryColor),
    accentColor: valueOrDefault(apiConfig.style?.accentColor, d.style.accentColor),
    widgetBgColor: valueOrDefault(apiConfig.style?.widgetBgColor, d.style.widgetBgColor),
    textColor: valueOrDefault(apiConfig.style?.textColor, d.style.textColor),
    userBubbleColor: valueOrDefault(apiConfig.style?.userBubbleColor, d.style.userBubbleColor),
    botBubbleColor: valueOrDefault(apiConfig.style?.botBubbleColor, d.style.botBubbleColor),
    buttonColor: valueOrDefault(apiConfig.style?.buttonColor, d.style.buttonColor),
    borderColor: valueOrDefault(apiConfig.style?.borderColor, d.style.borderColor),
    headerColor: valueOrDefault(apiConfig.style?.headerColor, d.style.headerColor),
    iconColor: valueOrDefault(apiConfig.style?.iconColor, d.style.iconColor),
    borderRadius: valueOrDefault(apiConfig.style?.borderRadius, d.style.borderRadius),
    buttonRadius: valueOrDefault(apiConfig.style?.buttonRadius, d.style.buttonRadius),
    shadow: valueOrDefault(apiConfig.style?.shadow, d.style.shadow),
    widgetWidth: valueOrDefault(apiConfig.style?.widgetWidth, d.style.widgetWidth),
    maxHeight: valueOrDefault(apiConfig.style?.maxHeight, d.style.maxHeight),
    widgetPosition: valueOrDefault(apiConfig.style?.widgetPosition, d.style.widgetPosition),
    padding: valueOrDefault(apiConfig.style?.padding, d.style.padding),
    fontFamily: valueOrDefault(apiConfig.style?.fontFamily, d.style.fontFamily),
    fontSize: valueOrDefault(apiConfig.style?.fontSize, d.style.fontSize),
    fabStyle: valueOrDefault(apiConfig.style?.fabStyle, d.style.fabStyle),
    ...
    tone: valueOrDefault(apiConfig.content?.tone, d.content.tone),
    humanFallbackCta: valueOrDefault(apiConfig.content?.humanFallbackCta, d.content.humanFallbackCta),
    hours: valueOrDefault(apiConfig.content?.hours, d.content.hours),
    address: valueOrDefault(apiConfig.content?.address, d.content.address),
    contact: valueOrDefault(apiConfig.content?.contact, d.content.contact),
    systemPrompt: valueOrDefault(apiConfig.systemPrompt, DEFAULT_SYSTEM_PROMPT),
    docs: apiConfig.docs ? JSON.stringify(apiConfig.docs) : JSON.stringify(d.docs),
    calendarProvider: valueOrDefault(apiConfig.calendarProvider, d.calendarProvider),
...
function unflattenConfig(dbConfig: PrismaChatbotConfig): AppChatbotConfig {
  const d = defaultChatbotConfig;
  return {
    id: dbConfig.id || d.id,
    branding: {
    ...
    style: {
      ...
      widgetPosition: dbConfig.widgetPosition === "left" || dbConfig.widgetPosition === "right"
        ? dbConfig.widgetPosition
        : d.style.widgetPosition,
...
export async function PUT(request: NextRequest) {
  try {
    const body = (await request.json()) as Partial<AppChatbotConfig>;
    const botId = body.id || "clarissa-v1";
    const flat = flattenConfig(body);

    const flatWithoutId = Object.fromEntries(
      Object.entries(flat).filter(([key]) => key !== "id")
    ) as Omit<typeof flat, "id">;
